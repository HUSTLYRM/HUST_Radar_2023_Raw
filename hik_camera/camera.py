...
from hik_camera.MvImport import MvCameraControl_class as hk
...
def camera_init(cam_id=''):
    ...
    for i in range(0, device_list.nDeviceNum):
        ...
        if mvcc_dev_info.nTLayerType == hk.MV_USB_DEVICE:
            ...
            # 根据硬件id判断左右相机
            if str_serial_number.endswith(cam_id[-2:]):
                n_connection_num = i

    if n_connection_num < 0:
        print("no correct camera")
        return
    ...
    _ret = _cam.MV_CC_CreateHandle(st_device_list)
    if _ret != 0:
        print("create handle fail! ret[0x%x]" % _ret)
        sys.exit()

    # 打开设备 此时并不读取输入流
    _cam.MV_CC_OpenDevice(hk.MV_ACCESS_Exclusive, 0)
    # 清空原有缓存(可能存在)
    _cam.MV_CC_ClearImageBuffer()

    return _cam


def set_parameters(_cam=hk.MvCamera(), _name='supporter', _cfg=None):
    if _cfg is None:
        print('given no cfg')
        sys.exit()
    # 设置触发模式为on 用于时间同步
    ...
    if _ret != 0:
        ...
        sys.exit()
    # 设置触发源 设置触发源为软触发
    _ret = _cam.MV_CC_SetEnumValue("TriggerSource", hk.MV_TRIGGER_SOURCE_SOFTWARE)  #hk.MV_TRIGGER_SOURCE_LINE2
    # LINE2 hk.MV_TRIGGER_SOURCE_SOFTWARE
    if _ret != 0:
        print("[%s] set trigger source fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    # 设置尺寸
    _ret = _cam.MV_CC_SetIntValue("Width", _cfg['param']['Width'])
    if _ret != 0:
        print("[%s] set width fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    _ret = _cam.MV_CC_SetIntValue("Height", _cfg['param']['Height'])
    if _ret != 0:
        print("[%s] set height fail! ret[0x%x]" % (_name, _ret))
        sys.exit()

    _ret = _cam.MV_CC_SetIntValue("OffsetY", _cfg['param']['OffsetY'])
    if _ret != 0:
        print("[%s] set offset(y) fail! ret[0x%x]" % (_name, _ret))
        sys.exit()

    # 设置曝光时间
    _ret = _cam.MV_CC_SetFloatValue("ExposureTime", _cfg['param']['ExposureTime'])
    if _ret != 0:
        print("[%s] set exposure fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    # 设置gamma矫正
    _ret = _cam.MV_CC_SetBoolValue("GammaEnable", _cfg['param']['GammaEnable'])
    if _ret != 0:
        print("[%s] set gammaEnable fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    _ret = _cam.MV_CC_SetFloatValue("Gamma", _cfg['param']['Gamma'])
    if _ret != 0:
        print("[%s] set gamma fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    # 设置 白平衡，用于图像颜色“不正”时
    _ret = _cam.MV_CC_SetEnumValueByString("BalanceWhiteAuto", _cfg['param']['BalanceWhiteAuto'])
    if _ret != 0:
    ...
    if _ret != 0:
        ...
        sys.exit()
    _ret = _cam.MV_CC_SetIntValue("BalanceRatio", _cfg['param']['BalanceRatioR'])
    if _ret != 0:
    ...
    if _ret != 0:
        ...
        sys.exit()
    _ret = _cam.MV_CC_SetIntValue("BalanceRatio", _cfg['param']['BalanceRatioG'])
    if _ret != 0:
        print("[%s] set balanceRatio fail! ret[0x%x]" % (_name, _ret))
        sys.exit()
    ...
    if _ret != 0:
        ...
        sys.exit()
    _ret = _cam.MV_CC_SetIntValue("BalanceRatio", _cfg['param']['BalanceRatioB'])
    if _ret != 0:
        print("[%s] set balanceRatio fail! ret[0x%x]" % (_name, _ret))
        sys.exit()


def camera_close(_cam):
...
def get_camera(cfg):
    # 寻找、连接并开启相机 (获取句柄)
    cam = camera_init(cfg['id'])
    print(cam.MV_CC_IsDeviceConnected())

    # 设置参数
    set_parameters(cam, 'camera', cfg)
