...
class CoExMatcher:
    ...
    def inference(self, left_img, right_img):
        ...
        # cv2.imshow("rel", left)
        # cv2.imshow("rer", right)

        # left = cv2.resize(left, (1216, 352))
        # right = cv2.resize(right, (1216, 352))
        # re_right = right
        # re_left = left
        ...
        # torch.unsqueeze(src_img, idx)
        # np.expand_dims(src_img, idx)
        # print(left_img_p.shape)
