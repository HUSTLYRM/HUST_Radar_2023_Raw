...
class CameraPoseSolver:
    ...
    def init_by_anchor(self, anchor):
        ...
        if self.color == RED:
            ...
            # t_l t_r b_r b_l: clockwise begin at top left vertex


            # top_left = [12000, 4500 + 297, + 210]
            # top_right = [12000, 4500, + 210]
            # bottom_right = [12000, 4500, 0]
            # bottom_left = [12000, 4500 + 297, 0]
        ...
        # self.main_pose_R = cv2.Rodrigues(rotation_vector)
        # np.transpose()
...
# TODO:
    def init_by_constant(self, constant_dict):
        ...
        # # 把相机坐标变为世界坐标的变换 就是 把世界坐标系变换为相机坐标系的变换？
        # roll = constant_dict['roll'] * np.pi / 180
        # yaw = constant_dict['yaw'] * np.pi / 180
        # pitch = constant_dict['pitch'] * np.pi / 180
        #
        #
        # self.main2field_R = euler2rotation(roll, yaw, pitch)
        # self.main2field_t = np.array(constant_dict['pose_T'])
        pass
    ...
    def get_field(self, cam_coord):
        field_coord = np.matmul(self.main2field_R, cam_coord)
        field_coord = field_coord + self.main2field_t
        return field_coord
    ...
    # TODO: 数据格式定义 说明文档
    def compensate_for_map(self, field_coord):
        map_coord = None
        pass
