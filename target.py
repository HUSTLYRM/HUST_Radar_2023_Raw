...
class Targets:
    ...
    def update_with_trackerpool(self, boxes, frame):
        ...
        """for car in cars:
            if len(car.armors) == 0:
                car.conf = 0
            # idx = car.get_idx(self.color)
            if car.conf > 0:
                idx = car.get_idx(self.enemy_color)
                if car.conf > self.targets[idx].conf:
                    self.targets[idx] = car
                    updated[idx] = True
                    self.multi_tracker.add(cv2.TrackerMIL_create(), frame, car.bbox)"""
